import { ArrowRight, ImageIcon, Boxes, Film, Mic, MessageSquare, Bot, BookOpen, Layers, Cpu } from 'lucide-react'
...
const HIGHLIGHTS = [
  { icon: BookOpen, title: '看得懂', desc: '完整的参考实现作为标准答案，配套讲义拆解 Agent 架构与编排逻辑。' },
  { icon: Layers, title: '改得动', desc: '在现成工程上做有梯度的动手任务：换模型、加工具、调编排、写 Skill。' },
  { icon: Cpu, title: '能落地', desc: '覆盖云 API 接入与开源模型私有化部署，对接真实多模态 Agent 开发岗位。' },
]
  ...
  return (
    <div className="tp-home">
      <header className="tp-hero">
        <div className="tp-hero-badge">企业级实训课程 · 多模态智能体开发</div>
        <h1 className="tp-hero-title">
          多模态智能体开发
          <span className="tp-hero-accent">实训平台</span>
        </h1>
        <p className="tp-hero-desc">
          以多模态融合为核心、Agent 自主决策为目标、实战落地为导向。在一个真实可运行的多模态 Agent 工程上，
          逐个项目动手实训，掌握从工具调用、多模态编排到工程化部署的核心技能。
        </p>
      ...
      <section className="tp-section">
        ...
        <div className="tp-highlights">
          {HIGHLIGHTS.map((h) => (
            <div className="tp-card tp-highlight" key={h.title}>
              <span className="tp-highlight-icon">
                <h.icon size={20} />
              </span>
              <h3>{h.title}</h3>
              <p>{h.desc}</p>
      ...
      <section className="tp-section">
        ...
        <div className="tp-projects">
          {PROJECTS.map((p) => (
            <button
              key={p.title}
              className={`tp-card tp-project ${p.ready ? '' : 'disabled'}`}
              onClick={() => p.ready && p.view && onEnter(p.view)}
              disabled={!p.ready}
            >
              <div className="tp-project-top">
                <span className="tp-project-icon">
                  <p.icon size={22} />
                </span>
                <span className="tp-project-chapter">{p.chapter}</span>
              </div>
              <h3 className="tp-project-title">{p.title}</h3>
              <p className="tp-project-desc">{p.desc}</p>
              ...
              {p.ready && (
                <span className="tp-project-enter">
      ...
      <footer className="tp-foot">
        PolyStudio 实训平台 · 共用同一个全能多模态 Agent 内核，按能力拆分为独立教学工作台
      </footer>
    </div>
  )
}
