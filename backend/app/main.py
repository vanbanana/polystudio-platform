...
"""
# 兼容 ormsgpack 新版 Reviver 接口（langgraph-checkpoint 4.x 兼容补丁）
try:
    import ormsgpack
    _orig_reviver_init = ormsgpack.Reviver.__init__
    def _patched_reviver_init(self, *args, **kwargs):
        if 'allowed_objects' in kwargs and 'allowed_classes' not in kwargs:
            kwargs['allowed_classes'] = kwargs.pop('allowed_objects')
        return _orig_reviver_init(self, *args, **kwargs)
    ormsgpack.Reviver.__init__ = _patched_reviver_init
except Exception:
    pass
